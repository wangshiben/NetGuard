...
public class SM2 {
	...
	public String decryptAsString(byte[] encryptData,BigInteger privateKey){
		try {
			return new String(	this.decrypt(encryptData,privateKey),"UTF8");
	 ...
	 * @return
	 */
	public SM2KeyPair generateKeyPair(BigInteger d) {
...
//		BigInteger d = random(n.subtract(new BigInteger("1")));
	...
	public static ECPoint initPubKey(BigInteger gx,BigInteger gy){
		ECCurve.Fp curve = new ECCurve.Fp(p, // q
				a, // a
				b); // b
		ECPoint res = curve.createPoint(gx, gy);
		return res;
	}
	...
	public Signature sign(String M, String IDA, SM2KeyPair keyPair) {
		return sign(M.getBytes(), IDA, keyPair);
	}
